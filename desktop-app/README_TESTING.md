...
This file is the short checklist for Git behavior in BranchTracker. Tests should prove each action works and that unsafe actions either warn first or cannot hide commits from the visible graph.
...
- Ask for an optional commit message before creating a BranchTracker branch and committing dirty cwd values with no known branch target.
