...
test("starts a terminal session in a cwd and emits output", async () => {
  ...
  const cwd = await mkdtemp(join(tmpdir(), "branchtracker-terminal-"));
  const expectedOutput = "branchtracker-terminal-test";
...
test("marks a terminal session busy while a child process is running", async () => {
  ...
  const cwd = await mkdtemp(join(tmpdir(), "branchtracker-terminal-busy-"));
  const sleepCommand =
...
test(
  ...
  async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), "branchtracker-terminal-tree-"));
    const childPidMarker = "branchtracker-terminal-child-pid";
